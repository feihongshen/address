...
public class AddressImportService extends CommonServiceImpl<AddressImportDetail, Long> {
    ...
    /**
     * 导入单行记录（新事务处理） 地址格式是否正确（ - -大望路）否-->失败 ：上级地址不存在 三级地址是否存在（北京-市辖区-朝阳区） 否-->失败：上级地址不存在 四、伍、六级地址是否存在（不存在：创建
     * [如果是最后一级，]，存在：添加至地址MAP） 站点非空（绑定判别-抛异常站点不存在） 配送员非空（绑定判别-为空抛异常配送员不存在）
     * @param map 名称-Address
     * @param detail 但行记录
     * @param addressMap 已经查出来的Map
     * @param customerId
     */
    public void txNewImportDetail(Map<String, Address> map, AddressImportDetail detail,
            Map<String, Address> addressMap, Map<String, DeliveryStation> stationMap,
            Map<String, Deliverer> delivererMap, Map<Long, Address> bindMap, Long customerId, Integer importType,
            Long stationId) {
        if (this.validateDetail(detail)) {
            ...
            if (d == null) {
                ...
                if (a1 == null) {// 为空则创建并绑定
                    a1 = this.createAndBind(d, detail.getAddress1(), customerId);
                    isSaved = true;
                } else {
                ...
                if (importType == AddressImportTypeEnum.init.getValue()) {// 初始化导入：绑定站点和小件员
                    DeliveryStation ds = null;
                    if (StringUtils.isNotBlank(detail.getDeliveryStationName())) {
                        ds = stationMap.get(customerId + "-" + detail.getDeliveryStationName());
                        if (ds == null) {
                            ...
                            this.deliveryStationRuleService.addRule(dsr, customerId);
                            isSaved = true;
                        }
                    }
                    if (StringUtils.isNotBlank(detail.getDelivererName())) {// 绑定小件员
                        Deliverer dl = delivererMap
                                .get(customerId + "-" + ds.getId() + "-" + detail.getDelivererName());
                        if (dl != null) {
                            DelivererRule dr = new DelivererRule();
                            dr.setAddress(bindAddress);
                            dr.setCreationTime(new Date());
                            dr.setDeliverer(dl);
                            dr.setDeliveryStation(ds);
                            dr.setRule("");
                            dr.setRuleExpression("");
                            dr.setRuleType(DelivererRuleTypeEnum.fallback.getValue());
                            this.delivererRuleService.addRule(dr, customerId);
                            isSaved = true;
                        } else {
                            throw new ExplinkRuntimeException("通过客户id[" + customerId + "]+站点ID[" + ds.getId()
                                    + "]+小件员[" + detail.getDelivererName() + "]获取小件员失败.");
                        }
                    }
                }
     ...
     * @param customerId
     */
    public void txNewMoveDetail(Map<String, Address> map, AddressImportDetail detail, Map<String, Address> addressMap,
            Map<String, DeliveryStation> stationMap, Map<String, Deliverer> delivererMap, Map<Long, Address> bindMap,
            Long customerId) {
        if (this.validateDetail(detail)) {
            Address d = map.get(detail.getProvince() + "-" + detail.getCity() + "-" + detail.getDistrict());
            if (d == null) {
                    ...
                    if (StringUtils.isNotBlank(detail.getDeliveryStationOldName())) {
                        ...
                        if (oldDs == null) {
                            ...
                            if (ds == null) {
                                ...
                                int count = this.deliveryStationRuleService.removeAddressRule(bindAddress.getId(),
                                        oldDs.getId(), ds.getId());
                                if (count > 0) {
                                    isSaved = true;
                                }
                            }
    ...
    private Address createAndBind(Address parent, String name, Long customerId) {
        ...
        a1 = this.addressDao.save(a1);
        AddressPermission permission = new AddressPermission();
        permission.setAddressId(a1.getId());
        permission.setCustomerId(customerId);
        this.addressPermissionDao.save(permission);
        this.scheduledTaskService.createScheduledTask(Constants.TASK_TYPE_SUB_UPDATE_INDEX,
                Constants.REFERENCE_TYPE_ADDRESS_ID, String.valueOf(a1.getId()));
        return a1;
    }
    ...
    private boolean validateDetail(AddressImportDetail detail) {
        ...
        if (StringUtils.isBlank(detail.getProvince()) || StringUtils.isBlank(detail.getCity())
                || StringUtils.isBlank(detail.getDistrict())) {
            return false;
        } else {
    ...
    private void bindRule(AddressImportDetail detail, Long customerId) {
        if (!detail.getStatus().equals(AddressImportDetailStatsEnum.failure.getValue())) {
            ...
            if (StringUtils.isNotEmpty(detail.getDeliveryStationName())) {
                ds = this.deliveryStationService.getByNameAndCustomerId(detail.getDeliveryStationName(), customerId);
                if (ds != null) {
                    DeliveryStationRule dsr = new DeliveryStationRule();
            ...
            if (StringUtils.isNotEmpty(detail.getDelivererName())) {
                Deliverer d = this.delivererService.getByCodeAndCustomerId(detail.getDelivererName(), customerId,
                        ds.getId());
                if (d != null) {
                    DelivererRule dr = new DelivererRule();
                    Address a = new Address();
    ...
    private void removeRule(AddressImportDetail detail, Long customerId) {
        if (!detail.getStatus().equals(AddressImportDetailStatsEnum.failure.getValue())) {
            // 站点规则迁移
            if (StringUtils.isNotEmpty(detail.getDeliveryStationName())) {
                DeliveryStation ds = this.deliveryStationService.getByNameAndCustomerId(
                        detail.getDeliveryStationName(), customerId);
                if (ds != null) {
                    this.deliveryStationRuleService.removeAddressRule(detail.getAddressId(), ds.getId());
                }
            }
        }
    }
    ...
    private void bindRuleWithStationId(AddressImportDetail detail, Long customerId, Long stationId) {
        if (!detail.getStatus().equals(AddressImportDetailStatsEnum.failure.getValue())) {
            ...
            // 小件员规则
            if (StringUtils.isNotEmpty(detail.getDelivererName())) {
                Deliverer d = this.delivererService.getByNameAndCustomerId(detail.getDelivererName(), customerId);
                if (d != null) {
                    DelivererRule dr = new DelivererRule();
                    Address a = new Address();
                    a.setId(detail.getAddressId());
     ...
     * @return
     */
    private TreeNode<AddressImportEntry> getTreeNode(TreeNode<AddressImportEntry> treeNode,
            Map<String, List<Address>> addressMap, AddressImportDetail detail, int level, Long customerId) {
        String name = this.getAddressName(detail, level);
        String nextLevelName = this.getAddressName(detail, level + 1);
        // 是否最后一级地址
        ...
        if (childAddress == null) {
            if (isLast) {
                // 如果是当前已是最后一级地址，创建新地址
                childAddress = this.createAddress(parentAddress, detail, name, customerId);
                childEntry.setAddress(childAddress);
                detail.setStatus(AddressImportDetailStatsEnum.success.getValue());
            ...
            if (isLast) {
                // 最后一级地址已存在
                boolean bindResult = this.addressService.bindAddress(childAddress, customerId);
                if (bindResult) {
                    detail.setStatus(AddressImportDetailStatsEnum.success.getValue());
                    detail.setAddressId(childAddress.getId());
                } else {
                ...
                // 不是最后一级，继续查找子节点
                return this.getTreeNode(childNode, addressMap, detail, level + 1, customerId);
            }
    ...
    private Address createAddress(Address parentAddress, AddressImportDetail detail, String name, Long customerId) {
        ...
        this.addressService.createAndBindAddress(address, parentAddress, customerId);
        return address;
    }
    ...
    public List<AddressImportResult> getImportAddressResults(Date startDate, Date endDate) {
        return this.addressImportResultDao.getImportAddressResults(startDate, endDate);
    }
    ...
    public void deleteImportAddressResult(Long resultId, Long customerId) {
        AddressImportResult addressImportResult = this.addressImportResultDao.get(resultId);
        ...
        if (addressIdList.size() > 0) {
            List<String> appendedChildrenIdList = this.addressService.findCannotRemoveIds(addressIdList, customerId);
            ...
            if (addressIdList.size() > 0) {
                this.addressService.batchUnbindAddress(addressIdList, customerId);
            }
        }
    ...
    public List<AddressImportDetail> getAllDetail() {
        return this.addressImportDetailDao.loadAll(AddressImportDetail.class);
