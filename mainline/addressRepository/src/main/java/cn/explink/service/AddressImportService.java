...
public class AddressImportService extends CommonServiceImpl<AddressImportDetail, Long> {
	 ...
	 * @param customerId
	 */
	public void txNewImportDetail(Map<String, Address> map, AddressImportDetail detail, Map<String, Address> addressMap, Map<String, DeliveryStation> stationMap, Map<String, Deliverer> delivererMap,
			Map<Long, Address> bindMap, Long customerId, Integer importType, Long stationId) {
	 ...
	 * @param customerId
	 */
	public void txNewMoveDetail(Map<String, Address> map, AddressImportDetail detail, Map<String, Address> addressMap, Map<String, DeliveryStation> stationMap, Map<String, Deliverer> delivererMap,
			Map<Long, Address> bindMap, Long customerId) {
		if (this.validateDetail(detail)) {
			Address d = map.get(detail.getProvince() + "-" + detail.getCity() + "-" + detail.getDistrict());
			if (d == null) {
				throw new ExplinkRuntimeException("省/市/区地址不存在");
					...
					if (StringUtils.isNotBlank(detail.getDeliveryStationOldName())) {
						...
						if (oldDs == null) {
							...
							if (ds == null) {
								...
								int count = this.deliveryStationRuleService.removeAddressRule(bindAddress.getId(), oldDs.getId(), ds.getId());
								if (count > 0) {
									isSaved = true;
								}
							}
						...
						if (ds == null) {
							...
							int count = this.deliveryStationRuleService.removeAddressRule(bindAddress.getId(), ds.getId());
							if (count > 0) {
								isSaved = true;
							}
						}
	...
	private Address createAndBind(Address parent, String name, Long customerId) {
		...
		permission.setCustomerId(customerId);
		this.addressPermissionDao.save(permission);
		this.scheduledTaskService.createScheduledTask(Constants.TASK_TYPE_SUB_UPDATE_INDEX, Constants.REFERENCE_TYPE_ADDRESS_ID, String.valueOf(a1.getId()));
		return a1;
	}
	...
	private void bindRule(AddressImportDetail detail, Long customerId) {
		if (!detail.getStatus().equals(AddressImportDetailStatsEnum.failure.getValue())) {
			...
			if (StringUtils.isNotEmpty(detail.getDeliveryStationName())) {
				DeliveryStation ds = this.deliveryStationService.getByNameAndCustomerId(detail.getDeliveryStationName(), customerId);
				if (ds != null) {
					DeliveryStationRule dsr = new DeliveryStationRule();
