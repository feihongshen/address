	...
	$("#station_edit").click(function()
	{
		...
		if(selections && selections.length > 0)
		{
			...
			else
			{
				...
				// 同步ajax调用
				Tools.doAction(ctx+'/station/listAll', params, false, function(data)
				{
					if(data)
					{
						for (var inte = 0, length = data.length; inte < length; inte++)
						{
							...
							{
								// 根据id排除当前选中对象
								existsRegions.push(data[inte].coordinate);
							}
						}
					}
				});
				...
				// 更新区域名称的显示
				$("#station_add_panel_input_area").val(selections[0].name);
	...
	$("#station_add_panel_btn_submit").click(function()
	{
		var selections = $("#stationList").datagrid("getSelections");
		var id = selections[0].id;
		
		var polygon = mapManager.getNewDrawRegion(), stationName = $("#station_add_panel_input_area").val();
		...
		if(!stationName)
		{
			layer.tips('请填写站点名称', document.getElementById("station_add_panel_input_area"), {
				guide : 1,
				time : 2
			});
			return;
	...
	function initializeStyleOptions()
	{
		...
		lineC.setAlpha(0.6);
		$("#station_add_panel_top_linecolor").spectrum({
			color : lineC,
			showAlpha : true,
			chooseText : "应用",
			cancelText : "取消",
			change : function(color)
			{
				...
				var fillColor = $("#station_add_panel_top_fillcolor").spectrum('get');
		...
		fillC.setAlpha(0.2);
		$("#station_add_panel_top_fillcolor").spectrum({
			color : fillC,
			showAlpha : true,
			chooseText : "应用",
			cancelText : "取消",
			...
			{
				var lineWidth = $('#station_add_panel_top_linewidth').val();
				var lineColor = $("#station_add_panel_top_linecolor").spectrum('get');
				setStyleOptions(lineColor.toHexString(), lineColor.getAlpha(), lineWidth, color.toHexString(), color.getAlpha());;
			}
		});

		// 线宽改变
		$('#station_add_panel_top_linewidth').change(function(e)
		{
			var lineWidth = $('#station_add_panel_top_linewidth').val();
			var fillColor = $("#station_add_panel_top_fillcolor").spectrum('get');
			var lineColor = $("#station_add_panel_top_linecolor").spectrum('get');
