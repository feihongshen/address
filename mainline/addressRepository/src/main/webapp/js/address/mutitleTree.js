...
Array.prototype.toSource = function() {
	...
	for (var i = 0; i < this.length; i++) {
		str += "{id:\"" + this[i].id + "\",pId:\"" + this[i].pId + "\",name:\""
				+ this[i].name + "\",isParent:\"" + this[i].isParent
				+ "\",file:\"" + this[i].file
				// +"\",icon:\""+this[i].icon
				+ "\",open:\"" + this[i].open + "\"},";
	}
...
function findParentNodes(treeNode, parentNodes) {
	parentNodes += "{id:" + treeNode.id + ",pId:" + treeNode.pId + ",name:\""
			+ treeNode.name + "\",open:" + treeNode.open + "},";
...
function moveNodes(zTreeFrom, treeNode, zTreeTo, divStrFrom, divStrTo) {
	...
	// /////////////////////////////
	var nodes = "[";
	nodes += "{id:" + treeNode.id + ",pId:" + treeNode.pId + ",name:\""
			+ treeNode.name + "\",open:" + treeNode.open + "},";
						...
						function() {
							...
							for (var i = 0; i < nodes.length; i++) {
								...
								if (treeNode.check_Child_State != 1) {
									var test = zTreeObj1.getNodeByParam("id",
											treeNode.id);
									if (test) {
										moveNodes(zTreeObj1, treeNode,
												zTreeObj2, leftDivStr,
												rightDivStr);
										reSearch();
									}
								}
				...
				$("#toLeft").click(
						function() {
							...
							for (var i = 0; i < nodes.length; i++) {
								var treeNode = nodes[i];
								if (treeNode.check_Child_State != 1) {
									var test = zTreeObj2.getNodeByParam("id",
											treeNode.id);
									if (test) {
										moveNodes(zTreeObj2, treeNode,
												zTreeObj1, rightDivStr,
												leftDivStr);
										reSearch();
									}
								}
