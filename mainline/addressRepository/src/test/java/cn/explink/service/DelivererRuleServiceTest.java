...
public class DelivererRuleServiceTest extends BaseTestCase {
    ...
    public void testCreateDelivererRule() {
        ...
        DelivererVo delivererVo = this.prepareDeliverer(customer);
        Deliverer deliverer = this.delivererService.createDeliverer(delivererVo);

        String rule = null;
        DelivererRule fallbackRule = this.delivererRuleService.createDelivererRule(this.addressId, deliverer.getId(),
                customer.getId(), rule);
        ...
        try {
            fallbackRule = this.delivererRuleService.createDelivererRule(this.addressId, deliverer.getId(),
                    customer.getId(), rule);
        ...
        rule = "100|号";
        DelivererRule simpleRule = this.delivererRuleService.createDelivererRule(this.addressId, deliverer.getId(),
                customer.getId(), rule);
        assertNotNull("create simpleRule should be success", simpleRule);

        rule = "11-21单|号";
        simpleRule = this.delivererRuleService.createDelivererRule(this.addressId, deliverer.getId(), customer.getId(),
                rule);
        assertNotNull("create simpleRule should be success", simpleRule);
    ...
    public void testSearchDelivererRule() {
        ...
        // DelivererRule fallbackRule = delivererRuleService.createDelivererRule(addressId, deliverer.getId(),
        // customer.getId(), rule);
        // assertNotNull("create fallbackRule should be success", fallbackRule);
        //
        // rule = "100|号";
         ...
         * assertNotNull("ruleList should not be null", ruleList); assertEquals("only one rule mapped", 1,
         * ruleList.size()); assertEquals("应当匹配到正确的规则", "11-21单|号", ruleList.get(0).getRule());
         */
    }
